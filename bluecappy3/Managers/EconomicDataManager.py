...
class EconomicDataManager():
    ...
    def WriteXMLNode(self, node):
      ...
      # price data
      for name,price in self.metalPrices.items():
    ...
    def CalculateBeforeTaxCashFlow(self, problemManager, mineDataManager):
      ...
      inflation =  (1.0+self.inflation)**np.array( list(range( mineDataManager.theMiningSystem.mineLife)) )
    ...
    def CalculateTaxes(self, problemManager, mineDataManager):
      ...
      yearsRemaining = np.array( list(range( mineDataManager.theMiningSystem.mineLife ,0,-1)) ) 
    ...
    def CalculateNPV(self,ncf):
      ...
      """
      numYears = len(ncf)
      years = np.array(list(range(numYears))) + 1.0 # NB end of year discounting (and no year 0)
