...
class RegionalCalculationManager():
    ...
    def RunNPVcalculation(self,theProblemManager):
        ...
        for depthOfCover in coverDepths:
          ...
          discountFactors = (1+theProblemManager.theMineDataManager.theEconomicDataManager.discountRate)**np.array(list(range( int( np.ceil(mineLife) ))) )
        ...
        if doTimer:
            ...
            pl.bar(list(range(len(dts))),dts,align="center")
            pl.xticks(list(range(len(dts))), labels)


            pl.bar(list(range(1,len(dts))),dts[1:]-dts[:-1],color="r",align="center")
